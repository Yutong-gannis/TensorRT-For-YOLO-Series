...
int main(int argc, char** argv) {
  if (argc == 5 && std::string(argv[1]) == "-model_path" && std::string(argv[3]) == "-video_path") {
    char* model_path = argv[2];
    char* video_path = argv[4];
    float* Boxes = new float[200];
    int* BboxNum = new int[1];
    int* ClassIndexs = new int[50];
    ...
    std::cerr << "--> yolo -model_path ./output.trt -video_path ./test.mp4" << std::endl;
    return -1;
  }
}
